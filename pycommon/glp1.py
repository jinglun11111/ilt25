...
class Design: 
    def __init__(self, filename=None, down=1): 
        ...
        self._polygons = []
        self._mask = None   # NEW: holds image-based design
        ...
        if filename.lower().endswith(".png"):
            # --- NEW: load image directly ---
            from PIL import Image
            img = Image.open(filename).convert("L")
            arr = np.array(img)
            arr = (arr > 127).astype(np.float32)  # binarize 0/1
            if down > 1:
                arr = arr[::down, ::down]
            self._mask = arr
            self._is_image = True
    ...
    def mat(self, sizeX=2048, sizeY=2048, offsetX=512, offsetY=512):
        if hasattr(self, "_mask") and self._mask is not None:
            # Return the binary image, optionally resized to (sizeX, sizeY)
            img = self._mask
            # Optional: resize to match requested size
            if img.shape != (sizeX, sizeY):
                img = cv2.resize(img, (sizeY, sizeX), interpolation=cv2.INTER_NEAREST)
            return img
        else:
            # Original GLP polygon rasterization
